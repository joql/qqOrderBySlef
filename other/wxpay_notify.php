	...
	public function NotifyProcess($data, &$msg)
	{
		...
		if($data['return_code']=='SUCCESS'){
			if($data['result_code']=='SUCCESS'){
				$srow=$DB->get_row("SELECT * FROM shua_pay WHERE trade_no='{$data['out_trade_no']}' limit 1");

				if($srow['status']==0){
					$DB->query("update `shua_pay` set `status` ='1',`endtime` ='$date' where `trade_no`='{$data['out_trade_no']}'");
					processOrder($srow);
					$msg='OK';
					return true;
				}else{
