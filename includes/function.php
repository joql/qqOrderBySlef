...
function processOrder($srow) {
	global $DB,$date;
	$row=$DB->get_row("select * from shua_orders where tid='{$srow['tid']}' and qq='{$srow['qq']}' order by id desc limit 1");
	if($row['qq'] && $row['status']==0){
		$DB->query("update `shua_orders` set `value`=`value`+1 where `id`='{$row['id']}'");
	}else{
		$DB->query("insert into `shua_orders` (`tid`,`qq`,`value`,`addtime`,`status`) values ('".$srow['tid']."','".$srow['qq']."','1','".$date."','0')");
	}
}
?>